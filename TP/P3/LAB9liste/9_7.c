...
int verif_palindrom(LIST *lista)
{
  ...
  int copy=0,nr=creare_numar(lista),ogl=0;
  copy=nr;
  while(copy!=0)
    {
      ogl=ogl*10+copy%10;
...
int main()
{
  ...
  if(verif_palindrom(lista)==1)
    printf("CIFRELE FORMEAZA UN NR PALINDROM\n");
  else
